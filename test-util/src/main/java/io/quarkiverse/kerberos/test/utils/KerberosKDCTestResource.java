...
public class KerberosKDCTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    private static void startLdapServer() throws Exception {
        ...
        createPartition(dsf, schemaManager, "users", "ou=users,dc=quarkus,dc=io");
    ...
    private static void createPartition(final DirectoryServiceFactory dsf, final SchemaManager schemaManager, final String id,
        ...
        Partition p = pf.createPartition(schemaManager, null, id, suffix, 1000, workingDir.toFile());
    ...
    private static void startKDC() throws Exception {
        ...
        kdcServer.setSearchBaseDn("ou=users,dc=quarkus,dc=io");
        KerberosConfig config = kdcServer.getConfig();
        config.setServicePrincipal("krbtgt/QUARKUS.IO@QUARKUS.IO");
        config.setPrimaryRealm("QUARKUS.IO");

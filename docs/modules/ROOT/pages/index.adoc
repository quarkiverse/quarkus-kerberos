...
You can cast `Principal` to `quarkiverse.kerberos.KerberosPrincipal` and get a full `bob@EXAMPLE.COM` name and/or the realm part of the name, `EXAMPLE.COM`.

You can also access `org.ietf.jgss.GSSContext` context used during the negotiation process:

`GSSContext context = identity.getCredential(quarkus.quariverse.GSSContextCredential.class).getGSSContext();`.
...
You can customize this process by registering a custom `quarkiverse.kerberos.ServicePrincipalSubjectFactory`:

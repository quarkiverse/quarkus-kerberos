...
[Kerberos](https://www.kerberos.org/software/tutorial.html) is a network authentication protocol.
...
This extension supports `Kerberos version 5` with the `HTTP` [Negotiate](https://datatracker.ietf.org/doc/html/rfc4559) authentication scheme which is based on the `Simple And Protected Negotiate Mechanism` (`SPNEGO`) and the `Generic Security Services Application Program Interface` (`GSSAPI`).
...
2. Edit `/etc/krb5.conf` - either uncomment the configuration related to `EXAMPLE.COM` or add a new realm, example, `QUARKUS.COM`. Make sure the realm's `kdc` and `admin_server` properties point to `localhost`.

3. Create the database: `kdb5_util create -s`.

4. Start `kadmin.local` and add principals and keytabs in its command line:
...
5. start KDC: `systemctl start krb5kdc.service` and `systemctl start kadmin.service`

6. Prepare a service ticket for `bob`:

`kinit bob`

7. Create your Quarkus application which will use this extension. Lets assume it has a JAX-RS method with a `/api/users/me` path and which returns a user name. Update its `application.properties` to point to the service principal key tab: `quarkus.kerberos.keytab-path=/etc/service.keytab`.

8. Build and start the application and test it:
...
You can also access `org.ietf.jgss.GSSContext` context used during the negotiation process:

`identity.getCredential(quarkus.quariverse.GSSContextCredential.class)`.

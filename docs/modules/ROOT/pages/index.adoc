...
Next, use `kinit` to create a ticket granting ticket (`TGT`) for a selected user principal - the browser will use this `TGT`. If the default Kerberos KDC configuration at `/etc/krb5.conf` is used then you don't even need to restart a browser. If a custom configuration is used then point to it with `KRB5_CONFIG` and either update `~/.bashrc` or laucnch the browser from the shell where `KRB5_CONFIG` is set.
